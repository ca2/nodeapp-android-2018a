...
void ID3_TagImpl::ParseReader(ID3_Reader &reader,bool bSeekable) // if easily Randomly bSeekable, search tags at the end, otherwise only at the beginning
{
  ...
  if(bSeekable) // if easily Randomly bSeekable, search tags at the end, otherwise only at the beginning
  {
     cur = wr.setCur(end);
