...
static int truncate(const char *path, size_t length)
{
   ...
   return file_set_length(path,length);
...
// used for streaming:
size_t ID3_TagImpl::Link(ID3_Reader &reader,flags_t tag_types,bool bSeekable) // if easily Randomly bSeekable, search tags at the end, otherwise only at the beginning
{
  _tags_to_parse.set(tag_types);
  ...
  this->ParseReader(reader, bSeekable);

  return this->GetPrependedBytes();
}

size_t RenderV1ToFile(ID3_TagImpl& tag, std::fstream& file)
{
  if (!file)
  {
    return 0;
  ...
  else
  {
    ...
    file.seekg(0-ID3_V1_LEN, file::seek_end);
    ...
    {
      file.seekp(0-ID3_V1_LEN, file::seek_end);
    }
...
size_t RenderV2ToFile(const ID3_TagImpl& tag, std::fstream& file)
{
  ...
  {
    file.seekp(0, file::seek_begin);
    file.write(tagData, tagSize);
  }
  else
  {
    ID3String filename = tag.GetFileName();
    ID3String sTmpSuffix = ".XXXXXX";
    ...
    file.seekg(tag.GetPrependedBytes(), file::seek_begin);
...
flags_t ID3_TagImpl::Update(flags_t ulTagFlag)
{
  ...
  std::fstream file;
  ID3String filename = this->GetFileName();
...
flags_t ID3_TagImpl::Strip(flags_t ulTagFlag)
{
  ...
  if (ulTagFlag & ID3TT_PREPENDED & _file_tags.get())
  {
    ...
    file.seekg(this->GetPrependedBytes(), file::seek_begin);
    ...
    while (!file.eof())
    {
      ...
      size_t nBytesToRead = (size_t)MIN((unsigned int)(nBytesRemaining - nBytesCopied), (unsigned int)BUFSIZ);
#else
      ...
      if (nBytesRead > 0)
      {
        ...
        file.seekp(-offset, file::seek_current);
        file.write((char *)aucBuffer, nBytesRead);
        file.seekg(this->GetPrependedBytes(), file::seek_current);
