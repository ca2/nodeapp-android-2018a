...
size_t ID3_FieldImpl::Get(char* buf, size_t maxLen, size_t index) const
{
  ...
  if (this->GetType() == ID3FTY_TEXTSTRING &&
      ...
      buf != NULL && maxLen > 0)
  {
    ID3String data = this->GetTextItem(index);
...
ID3String ID3_FieldImpl::GetText() const
{
  ID3String data;
  if (this->GetType() == ID3FTY_TEXTSTRING)
  {
...
ID3String ID3_FieldImpl::GetTextItem(size_t index) const
{
  ID3String data;
  if (this->GetType() == ID3FTY_TEXTSTRING &&
      this->GetEncoding() == ID3TE_ASCII)
...
namespace
{
  ID3String getFixed(ID3String data, size_t size)
  {
    ID3String text(data, 0, size);
    if (compare::lt(text.size(), size))
    {
...
size_t ID3_FieldImpl::SetText_i(ID3String data)
{
  this->Clear();
  if (_fixed_size > 0)
  {
...
size_t ID3_FieldImpl::SetText(ID3String data)
{
  size_t len = 0;
  if (this->GetType() == ID3FTY_TEXTSTRING)
  {
 ...
 **/
size_t ID3_FieldImpl::AddText_i(ID3String data)
{
...
size_t ID3_FieldImpl::AddText(ID3String data)
{
  size_t len = 0;
  if (this->GetType() == ID3FTY_TEXTSTRING)
  {
...
namespace
{
  ID3String readEncodedText(ID3_Reader& reader, size_t len, ID3_TextEnc enc)
  {
    if (enc == ID3TE_ASCII)
    {
  ...
  ID3String readEncodedString(ID3_Reader& reader, ID3_TextEnc enc)
  {
    if (enc == ID3TE_ASCII)
    {
  ...
  size_t writeEncodedText(ID3_Writer& writer, ID3String data, ID3_TextEnc enc)
  {
    if (enc == ID3TE_ASCII)
    {
  ...
  size_t writeEncodedString(ID3_Writer& writer, ID3String data, ID3_TextEnc enc)
  {
    if (enc == ID3TE_ASCII)
    {
...
bool ID3_FieldImpl::ParseText(ID3_Reader& reader)
{
  ...
  if (fixed_size)
  {
    ...
    ID3String text = readEncodedText(reader, fixed_size, enc);
    this->SetText(text);
  ...
  else if (_flags & ID3FF_LIST)
  {
    ...
    while (!reader.atEnd())
    {
      ID3String text = readEncodedString(reader, enc);
      this->AddText(text);
  ...
  else
  {
    ...
    ID3String text = readEncodedText(reader, reader.remainingBytes(), enc);
    // not null terminated.
    this->AddText(text);
