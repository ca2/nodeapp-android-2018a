...
ID3String io::readString(ID3_Reader& reader)
{
  ID3String str;
  while (!reader.atEnd())
  {
...
ID3String io::readText(ID3_Reader& reader, size_t len)
{
  ID3String str;
  ...
  while (remaining > 0 && !reader.atEnd())
  {
    ...
    remaining -= numRead;
    memcpy(&psz[pos], buf, numRead);
    pos += numRead;
  }
  str.ReleaseBuffer(pos);
  return str;
}
...
ID3String io::readUnicodeString(ID3_Reader& reader)
{
  ID3String unicode;
...
ID3String io::readUnicodeText(ID3_Reader& reader, size_t len)
{
  ID3String unicode;
  ID3_Reader::char_type ch1, ch2;
...
uint32 io::readBENumber(ID3_Reader& reader, size_t len)
{
  ...
  for (ID3_Reader::size_type i = 0; compare::lt(i, len) && !reader.atEnd(); ++i)
  {
    val *= 256; // 2^8
...
ID3String io::readTrailingSpaces(ID3_Reader& reader, size_t len)
{
...
size_t io::writeTrailingSpaces(ID3_Writer& writer, ID3String buf, size_t len)
{
...
size_t io::writeString(ID3_Writer& writer, ID3String data)
{
...
size_t io::writeText(ID3_Writer& writer, ID3String data)
{
  ID3_Writer::pos_type beg = writer.getCur();
...
size_t io::writeUnicodeString(ID3_Writer& writer, ID3String data, bool bom)
{
...
size_t io::writeUnicodeText(ID3_Writer& writer, ID3String data, bool bom)
{
  ID3_Writer::pos_type beg = writer.getCur();
