...
namespace 
{
  ...
  ID3_Frame* readTextFrame(ID3_Reader& reader, ID3_FrameID id, const ID3String desc = "")
  {
...
bool mm::parse(ID3_TagImpl& tag, ID3_Reader& rdr)
{
  ...
  ID3String imgExt = io::readTrailingSpaces(dataWindow, 4);
  ...
  if (trkNum > 0)
  {
    ID3String trkStr = toString(trkNum);
