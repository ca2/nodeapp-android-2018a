 ...
 **   // for ascii strings
 **   char str1[1024];
 **   const char* p1 = "My ID3String";
 **   const char* p2 = "My Other ID3String";
 **
 **   myField->Set(p1);
 ...
 */
size_t ID3_Tag::Link(ID3_Reader &reader,flags_t flags,bool bSeekable) // if easily Randomly bSeekable, search tags at the end, otherwise only at the beginning
{
  return _impl->Link(reader, flags, bSeekable);
}
...
const char* ID3_Tag::GetFileName() const
{
  ...
  ID3String fn = _impl->GetFileName();
  if (fn.size())
  {
