...
namespace
{
  ...
  bool findText(ID3_Reader& reader, ID3String text)
  {
    if (text.empty())
    {
      return true;
...
bool lyr3::v1::parse(ID3_TagImpl& tag, ID3_Reader& reader)
{
  ...
  ID3String lyrics = io::readText(lfr, wr.remainingBytes());
...
bool lyr3::v2::parse(ID3_TagImpl& tag, ID3_Reader& reader)
{
  ...
  while (!wr.atEnd())
  {
    ...
    ID3String fldName = io::readText(wr, 3);
    ...
    else if (fldName == "LYR")
    {
      ...
      ID3String desc =  "Converted from Lyrics3 v2.00";
