...
//      DWORD dwTimeIn = GetTickCount();
      ...
      ::ca2::rgn & rgnClip = m_twrenderclienttool.GetClipRgn();
      rect rectClip;
/*      if(lpcrgn != NULL)
      {
         rgnClip.CopyRgn((::ca2::rgn *) lpcrgn);
         rect rect;
...
//      DWORD dwTime1 = GetTickCount();
   /*   TRACE("//\n");
      ...
      ::ca2::graphics * pdc = (dynamic_cast<::android::graphics * >(m_pbuffer->GetBuffer()));
...
/*      for(int i = hwnda.get_upper_bound(); i >= 0; i--)
      {
         ...
         {
         ::ca2::window * pwnd = dynamic_cast < ::ca2::window * > (window::FromHandlePermanent(hwnda[i]));
         if(pwnd == NULL)
         {
            for(int j = 0; j < wndpa.get_count(); j++)
            {
               ...
               {
                  pwnd = dynamic_cast < ::ca2::window * > (wndpa[j]->m_pimpl);
                  break;
               }
            }
         }
            ...
            ::SendMessageTimeout(
               hwnda[i],
               WM_PRINTCLIENT,
               (WPARAM) (HDC) *dynamic_cast < ::android::graphics *> (dib->get_graphics()),
               0,
               0,
               184,
               &dwptrResult);
            ...
            if(pwnd == NULL)
            {
               ::UpdateLayeredWindowX(hwnda[i], NULL, NULL, NULL,
                  *dynamic_cast < ::android::graphics *> (dib->get_graphics()),
...
//      DWORD dwTimeIn = GetTickCount();
   ...
   // The first ::ca2::window handle in the base_array must belong
...
//      DWORD dwTimeOut = GetTickCount();
      ...
      )
   {
      ...
      //
      // Routine:
      // hwndParam handle ::ca2::window
      // device context retrieve
      //
      //////////////////////////////////.

      // Try to get ::ca2::window owned
      // device context.
