...
namespace android
{
   ...
   bitmap* PASCAL bitmap::from_handle(::ca2::application * papp, HBITMAP hBitmap)
   {
      return dynamic_cast < bitmap * > (::android::graphics_object::from_handle(papp, hBitmap));
   }
   bitmap::~bitmap()
   { }
   ...
   BOOL bitmap::CreateDIBSection(::ca2::graphics * pdc, const BITMAPINFO * lpbmi, UINT usage, void **ppvBits, HANDLE hSection, DWORD offset)
   ...
   { return Attach(::CreateCompatibleBitmap((dynamic_cast<::android::graphics * >(pgraphics))->get_handle1(), nWidth, nHeight)); }
   BOOL bitmap::CreateDiscardableBitmap(::ca2::graphics * pgraphics, int nWidth, int nHeight)
   { return Attach(::CreateDiscardableBitmap((dynamic_cast<::android::graphics * >(pgraphics))->get_handle1(), nWidth, nHeight)); }
