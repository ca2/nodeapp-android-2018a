...
void CLASS_DECL_ANDROID __cdecl _ca2_purecall(void)
{
...
int CLASS_DECL_ANDROID AfxWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
...
::ca2::application * win_instantiate_application(::ca2::application * pappSystem, const char * pszId)
{
   ...
   {
#ifdef _M_X64
      lpgetnewapp = (LP_GET_NEW_APP) ::GetProcAddress(hmodule, "?get_new_app@@YAPEAVapplication@ca2@@XZ");
#else
      lpgetnewapp = (LP_GET_NEW_APP) ::GetProcAddress(hmodule, "?get_new_app@@YAPAVapplication@ca2@@XZ");
#endif
   }
   if(lpgetnewapp != NULL)
   {
